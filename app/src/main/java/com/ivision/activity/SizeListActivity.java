...
public class SizeListActivity extends BaseActivity {
    ...
    private String customerId = "", customerName = "", vendorId = "", millId = "", millName = "", productId = "", categoryId = "", millBasicPrice = "", basicPrice = "";
    ...
    private void init() {
        ...
        binding.llMill.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                goToActivityForResult(new Intent(context, SelectMillListActivity.class).putExtra("customerId", customerId).putExtra("customerName", customerName), new OnActivityResultLauncher() {
                    @Override
                    public void onActivityResultData(Intent data, int resultCode) {
                        if (resultCode == Activity.RESULT_OK) {
                            if (data != null) {
    ...
    private void bindMillName() {
        if (!millName.isEmpty()) binding.tvMillName.setText(millName);
        productId = "";
        categoryId = "";
        getProductList();
    }
    ...
    private void getSizeList(boolean loadMore) {
        ...
        ApiInterface apiInterface = ApiUtils.getApiCalling();
        apiInterface.getList(Constant.sizeListUrl, Common.getFranchiseId(), Common.getUserId(), customerId, vendorId,millId, productId, categoryId, String.valueOf(limit), String.valueOf(offset), Common.getUsername(), Common.getPassword()).enqueue(new Callback<JsonObject>() {
    ...
    private void bindData(JsonArray jsonArray) {
        ...
        adapter = new SizeListAdapter(context, list, new ClickListener() {
            ...
            public void onItemSelected(int position, Size model) {
                Common.confirmationDialog(SizeListActivity.this, context.getString(R.string.confirmation_remove), "No", "Yes", new Runnable() {
                    @Override
                    public void run() {
                        removeCart(position);
                    }
